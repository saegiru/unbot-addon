...
function CommandShowStrategyFrame(index)
	...
	if (not UnitIsPlayer("target")) then
		DisplayInfomation("You need to select a player bot as target.");
		return;
	end
	if (isParty == nil and isRaid == nil) then
		DisplayInfomation("Target is not in your party.");
		return;
	end
...
function CommandListIcons(index)
	...
	CreateIconsByUnBotBagsFrame(0, "UnBotBagsFrameIcon",0,false,iconsList,UnitName("player"),UnitClass("player"),"All Icons Display",nil,nil,GetIconFunc);
end
	...
	if (UnitInRaid("player") ~= nil) then
		if (GetNumRaidMembers()>0) then
			for i=1, GetNumRaidMembers(), 1 do
				...
				if (name) then
					DisplayInfomation(name.." in raid leaves combat");
		...
		if (GetNumPartyMembers()>0) then
			for i=1, GetNumPartyMembers(), 1 do
				...
				if (name) then
					DisplayInfomation(name.." in party leaves combat");
					SendChatMessage(UnBotExecuteCommand[index]..name, "PARTY");
				end
			end
		end
